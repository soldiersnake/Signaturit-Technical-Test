    ...
    return (
        <div className="card">
            ...
            <table className="table">
                ...
                <tbody>
                    {docs.map(d => {
                        const hasSigners = d.signers && d.signers.length > 0;
                        return (
                            <tr key={d.id}>
                                <td>
                                ...
                                <td className="row gap">
                                    <button
                                        onClick={() => set(d.id, 'signed')}
                                        disabled={!hasSigners}
                                    >Mark Signed
                                    </button>
                                    <button
                                        onClick={() => set(d.id, 'declined')} className="secondary"
                                        disabled={!hasSigners}
                                    >Mark Declined
                                    </button>
                                    <button
                                        onClick={() => set(d.id, 'pending')} className="ghost"
                                        disabled={!hasSigners}
                                    >Reset
                                    </button>
                                </td>
